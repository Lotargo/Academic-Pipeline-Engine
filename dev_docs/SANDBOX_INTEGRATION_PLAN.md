...
- [x] Разработка системы нумерации строк для контекста секций.
- [x] Реализация функции `replace_lines` для точечного патчинга контента.
- [x] Обновление шаблонов ревизий в `academic_pe/core/prompting.py` для поддержки строковых диапазонов в ответах агента.
- [x] Перенос логики обработки ошибок `SectionPatchError` на новую схему.

### Этап 2: Изолированная Python-песочница
- [x] Создание изолированного контейнера/процесса выполнения кода.
- [x] Подготовка базового окружения с предустановленными пакетами (`numpy`, `pandas`, `matplotlib`, `sympy`, `scipy`).
- [x] Написание промптов для написания исполняемого кода агентом.
